  ...
  return (
    <div className="space-y-12">
      ...
      <div className="text-center space-y-3 max-w-2xl mx-auto">
        ...
        <h1 className="text-4xl font-extrabold tracking-tight sm:text-5xl text-zinc-850 dark:text-zinc-100 text-zinc-900 dark:text-white">
      ...
      <div className="flex items-center justify-center space-x-2 text-xs font-medium text-zinc-500 dark:text-zinc-400 max-w-md mx-auto">
        ...
        <AnimatePresence mode="wait">
          ...
          {step === 1 && (
            ...
            >
              ...
              {isUploading && (
                <div className="flex flex-col items-center justify-center space-y-2 text-xs text-zinc-500 dark:text-zinc-400">
              ...
              {error && (
                <div className="text-center text-xs text-red-500 dark:text-red-400 bg-red-500/10 border border-red-500/20 max-w-md mx-auto p-3 rounded-xl">
                  {error}
                </div>
              )}
            </motion.div>
          ...
          {step === 2 && uploadData && (
            ...
            >
              {/* File Info Bar */}
              <div className="flex flex-col md:flex-row md:items-center justify-between p-4 gap-4 rounded-xl border border-border bg-card/20 backdrop-blur-md">
                <div className="flex items-center space-x-3">
                  <div className="p-2 rounded-lg bg-zinc-200/50 dark:bg-zinc-900 border border-border text-indigo-400 flex-shrink-0">
                    <FileSpreadsheet className="w-5 h-5" />
                  </div>
                  <div className="min-w-0">
                    <h3 className="text-xs font-semibold text-zinc-800 dark:text-zinc-200 truncate">{uploadData.filename}</h3>
                    <p className="text-[10px] text-zinc-550 dark:text-zinc-400">File loaded successfully • {uploadData.totalRows} rows parsed</p>
                  </div>
                </div>
                <div className="flex space-x-3 w-full md:w-auto">
                  <button
                    onClick={handleReset}
                    className="flex-1 md:flex-none px-4 py-2 rounded-lg border border-border hover:bg-zinc-100 dark:hover:bg-zinc-900 text-xs font-medium text-zinc-650 dark:text-zinc-400 hover:text-zinc-900 hover:dark:text-zinc-200 transition-colors"
                  >
                    Cancel
                  </button>
                  <button
                    onClick={handleConfirm}
                    className="flex-1 md:flex-none px-4 py-2 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-xs font-semibold text-white transition-all shadow-[0_0_15px_rgba(99,102,241,0.3)] hover:shadow-[0_0_20px_rgba(99,102,241,0.5)] flex items-center justify-center space-x-1.5"
                  >
          ...
          {step === 3 && (
            ...
            >
              ...
              <div className="space-y-2">
                <h3 className="text-lg font-bold text-zinc-800 dark:text-zinc-200">AI Semantic Mapping in Progress...</h3>
